...
def get_aggregators():
    """Returns a list of unique aggregator channel IDs or bot usernames."""
    channels = []
...
async def get_chat_link(client: TelegramClient, chat_id: int, message_id: int) -> str:
    """Generates a link to the original message."""
    try:
...
async def split_and_send(client, aggregator, chunks, reply_to=None):
    """Sends long text in multiple chunks."""
    last_sent = None
    for chunk in chunks:
        last_sent = await client.send_message(
            aggregator,
            chunk,
            reply_to=reply_to or last_sent,
            link_preview=False
        )
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if is_album:
                        ...
                        m_obj = media.media if hasattr(media, "media") else media
                        if m_obj and not isinstance(m_obj, MessageMediaWebPage):
                            has_media = True
                    ...
                    if is_album:
                        ...
                        if len(text) > limit:
                            ...
                            if not sent_msg.text and text:
                                await client.send_message(aggregator, text, reply_to=sent_msg.id)
                            else:
                                reply_to_id = sent_msg.id
                                await split_and_send(client, aggregator, remaining_chunks, reply_to=reply_to_id)
                        else:
                            sent = await client.send_file(aggregator, media_to_send, caption=text, reply_to=reply_to)
                            sent_msg = sent[0] if isinstance(sent, list) else sent
                            if not sent_msg.text and text:
                                await client.send_message(aggregator, text, reply_to=sent_msg.id)
                    ...
                    if has_media:
                        media_to_send = media.media if hasattr(media, "media") else media

                        if len(text) > limit:
                            caption = text[:limit-3] + "..."
                            remaining_chunks = [text[i:i + TEXT_LIMIT] for i in range(limit-3, len(text), TEXT_LIMIT)]

                            sent = await client.send_file(aggregator, media_to_send, caption=caption, reply_to=reply_to, buttons=buttons)
                            await asyncio.sleep(1)

                            if not sent.text and text:
                                await client.send_message(aggregator, text, reply_to=sent.id)
                            else:
                                await split_and_send(client, aggregator, remaining_chunks, reply_to=sent.id)
                        else:
                            sent = await client.send_file(aggregator, media_to_send, caption=text, reply_to=reply_to, buttons=buttons)
                            if not sent.text and text:
                                await client.send_message(aggregator, text, reply_to=sent.id)
                        ...
                        if len(text) > limit:
                            ...
                            sent = await client.send_message(aggregator, truncated, reply_to=reply_to, buttons=buttons, link_preview=False)
                            await asyncio.sleep(1)
                            await split_and_send(client, aggregator, remaining_chunks, reply_to=sent.id)
                        else:
...
async def register_handlers(client: TelegramClient, db: Database, active_ids: list, inactive_names: list):
    """Registers event handlers for the Telegram client."""
    aggregators = get_aggregators()
    if not aggregators:
    ...
    async def message_handler(event):
        if event.out and event.text.startswith('/status'):
            return
        await process_message(
            client, db, aggregators, event.chat_id, [event.message],
